...
runs `php bin/console some:command`, both `some:command` and `another:command` should be executed. Commands registered
as chain members can no longer be executed on their own, so `php bin/console another:command` should produce an error.
...
Implement detailed logging for chained commands, so that after execution of `php bin/console foo:hello`
